...
export default class extends Controller {
  ...
  removeFavorite(event) {
    ...
    fetch(`/product_favorites/${productFavoritId}`, {
      ...
      // - transform the toggle button so that it can now 'add' again the product into user favorite
      // - change the icon
  ...
  addFavorite(event) {
    ...
    // first check if the user is authorized to add product to its favorites (if he is signed in)
    ...
    // - transform the toggle button so that it can now 'remove' the product from user favorite
    // - change the icon
        .then((data) => {
          if (data.error) {
  ...
  removeFavoriteFromDashboard(event) {
    ...
    fetch(`/product_favorites/${productFavoritId}`, {
      ...
      // - check if there are still some product favorite in the dashboard,
      // if not reload the page to show the specific empty layout
